(function() {
  ...
  var turnTop = getElementTop($('.turn'));
  ...
  var timesFloat = getId('times-float')
  // 悬浮次数
  timesFloat.addEventListener('touchend', function() {
    scrollTo(0, turnTop)
  })
  // 获取铭牌
  ...
  for(var i = 0; i < voteBtn.length; i++) {
    (function(i) {
      ...
      voteBtn[j].addEventListener('touchend', function(event) {
        ...
        if ($('.vote-ticket').innerHTML==='<p>您今天还有0票可投</p>') {
          popUp('noticket')
        } else {
          addNameplate(addId)
          voteAdd[j].style.display = 'flex'
          setTimeout(function(){voteAdd[j].style.display = 'none'}, 500)
          setTimeout(function(){popUp('lottery')}, 600)
        }
      })
    })(i)
  }
  ...
  //弹框各项事件
  var activityRule = getId('activity-rule')
  var mask = getId('mask')
  ...
  activityRule.addEventListener('touchend', function() {
    popUp('explanation')
  })
  // 抽奖及抽奖活动说明弹框事件
  ...
  // 没有投票机会弹框事件
  noticketBtn.addEventListener('touchend', function() {
    hidePop('noticket')
  })
  // 没有抽奖机会弹框事件
  nolotteryBtn.addEventListener('touchend', function() {
    hidePop('nolottery')
  })
  //点击蒙层，消灭所有弹框
